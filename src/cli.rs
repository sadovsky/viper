...
fn gen(a: &Args) -> Result<()> {
    ...
    for i in 0..count {
        ...
        let header = format!(
            "# generated by viper gen — style {} v{} seed {} key {} progression {} form {} motif {} shape {} timbre {}\n# {} bars, {} unique phrases, lead density {:.2}, lead range {} semitones, {} drum hits\n",
            style.name, style.version, seed, song.key_name, info.progression, info.form,
            if info.motif { "on" } else { "off" }, info.motif_name, info.timbre,
            r.bars, r.unique_phrases, r.lead_density, r.lead_range, r.drum_hits
        );
