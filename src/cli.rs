...
usage:
  ...
  viper rip song.nsf|game.nes|writes.log [-o out.vip] [--bpm B] [--song N]
                [--frames N] [--press start@120] [--trace out.tsv]
      read music back out of a compiled NSF, out of a game cartridge by
      playing it, or out of any `frame addr value` register dump another
      emulator produced; without -o it only reports what it found.
      --bpm skips tempo detection; --press works a title screen.
...
fn gen(a: &Args) -> Result<()> {
    ...
    for i in 0..count {
        ...
        let header = gen_header_comment(&style.name, &style.version, seed, &song.key_name, &info, &r);
        ...
        println!(
            "{}\t{}\tseed={}\tkey={} {}\tbpm={}\tbars={}\ttimbre={}\tmotif={}\tdensity={:.2}\trange={}",
            path.display(), song.title, seed, crate::vip::key_name(info.key), info.scale, song.bpm, r.bars, info.timbre, info.motif_name, r.lead_density, r.lead_range
        );
    }
    Ok(())
}
...
fn compile_cmd(a: &Args) -> Result<()> {
    ...
    println!(
        "{}",
        compile_summary_line(module.songs.len(), &out.display().to_string(), bytes.len(), emitted.data_bytes, emitted.sample_bytes, total_frames)
    );
    Ok(())
}
...
/// `viper rip` — read music back out of a compiled NSF, or out of any
/// register dump another emulator produced.
fn rip(a: &Args) -> Result<()> {
    let path = a.positional.first().map(PathBuf::from).context("rip: need an .nsf or a register log")?;
    let bytes = std::fs::read(&path).with_context(|| format!("read {}", path.display()))?;
    let song_idx = a.num::<u8>("song")?.unwrap_or(0);
    let frames = a.num::<u32>("frames")?;

    // Three ways in, all ending at the same register log. An NSF is run by
    // the player; a cartridge is booted and listened to; a dump from another
    // emulator is simply read. Nothing downstream knows the difference,
    // which is the point — there is one transcriber, not three.
    let (trace, loop_frames, source) = if bytes.starts_with(b"NESM\x1A") || bytes.starts_with(b"NSFE") {
        let nsf = viper_apu::Nsf::parse(&bytes)?;
        let (t, looped) = viper_apu::trace::trace_nsf(&nsf, song_idx, frames)?;
        (t, looped, format!("{} song {}", path.display(), song_idx))
    } else if bytes.starts_with(b"NES\x1A") {
        // Ten seconds by default: long enough for a title theme to state
        // itself, short enough to be worth waiting for.
        let n = frames.unwrap_or(600);
        let presses = parse_presses(a.get("press"))?;
        let (_, log) = viper_apu::nes::run(&bytes, n, &presses)?;
        if log.is_empty() {
            bail!(
                "rip: {} wrote nothing to the sound chip in {} frames. Games do not \
                 play until something asks them to — try --press start@120, or a \
                 longer --frames.",
                path.display(),
                n
            );
        ...
        (t, None, format!("{} (played for {} frames)", path.display(), n))
    } else {
        let log = viper_apu::verify::parse_log(&String::from_utf8_lossy(&bytes));
        if log.is_empty() {
    ...
    song.title = path.file_stem().map(|s| s.to_string_lossy().to_string()).unwrap_or_default();

    if let Some(out) = a.get("out") {
        let text = vip::to_vip(&song);
        std::fs::write(out, provenance(&report, &path) + &text).with_context(|| format!("write {}", out))?;
        println!("→ {}", out);
    }
    print!("{}", report.summary());
    if a.get("out").is_none() {
        println!("(no -o given, so nothing was written)");
    }
    Ok(())
}

/// Parse `--press start@120,a@240` into a schedule of button presses.
///
/// A game sits on its title screen until something asks it to start, so
/// without this a capture only ever hears the title theme.
fn parse_presses(spec: Option<&str>) -> Result<Vec<viper_apu::nes::Press>> {
    let Some(spec) = spec else { return Ok(Vec::new()) };
    let mut out = Vec::new();
    for part in spec.split(',').filter(|p| !p.is_empty()) {
        let (name, at) = part.split_once('@').unwrap_or((part, "60"));
        let buttons = viper_apu::nes::button(name).ok_or_else(|| {
            anyhow!("--press: '{}' is not a button (a, b, select, start, up, down, left, right)", name)
        })?;
        let frame: u32 = at
            .parse()
            .map_err(|_| anyhow!("--press: '{}' is not a frame number", at))?;
        // Eight frames is a comfortable press: long enough that a game
        // polling once a frame cannot miss it, short enough not to read as
        // holding the button down.
        out.push(viper_apu::nes::Press { frame, buttons, hold: 8 });
    }
    Ok(out)
}

/// A header comment saying where the file came from and what was inferred.
/// A ripped song looks exactly like a hand-written one, and six months later
/// nobody remembers which is which.
fn provenance(r: &crate::rip::Report, src: &Path) -> String {
    format!(
        "# ripped by viper from {}\n\
         # {} frames, {} BPM ({:?} — see the rip report)\n\
         # Inferred, not read: tempo, phrase boundaries, instruments. The\n\
         # original instrument identities and any groove are not recoverable.\n\n",
        src.display(),
        r.frames,
        r.bpm,
...
mod tests {
    ...
    /// Parse a compile summary the way the downstream packer does: find the
    /// byte counts inside the parentheses. Deliberately a separate
    /// implementation rather than a call back into `cli`, because the point
    /// is to fail when the *shape* changes, not when the format string moves.
    fn scrape_compile_sizes(line: &str) -> Option<(usize, usize)> {
        let data = line.split(" song data").next()?.rsplit('(').next()?.trim().parse().ok()?;
        let samples = line.split(" samples)").next()?.rsplit(", ").next()?.trim().parse().ok()?;
        Some((data, samples))
    }

    #[test]
    fn the_compile_summary_still_yields_the_numbers_a_packer_needs() {
        // The downstream album build runs a throwaway compile of every song
        // purely to read these two counts back out of stdout, then bin-packs
        // tracks into 32 KB volumes. There is no --json, so this line is the
        // interface. Rewording "song data" to "data" breaks that build, and
        // this test is where that becomes visible.
        let line = compile_summary_line(1, "/tmp/out.nsf", 17601, 699, 1089, 196);
        assert_eq!(scrape_compile_sizes(&line), Some((699, 1089)));
        assert!(line.contains("17601 bytes"), "the total is there too: {}", line);
        assert!(line.contains("196 frames"), "{}", line);
        assert!(line.contains("3.3s"), "and a duration to one decimal: {}", line);
    }

    #[test]
    fn the_gen_header_marker_is_matched_verbatim_downstream() {
        // Curation scripts find generated songs by this exact prefix. It is
        // a marker rather than prose — including the em dash, which is the
        // character most likely to be "tidied" into a hyphen by someone
        // normalising punctuation, and doing so makes every song invisible
        // to the tools that pick them.
        assert_eq!(GEN_HEADER_MARKER, "# generated by viper gen — style ");
        assert!(GEN_HEADER_MARKER.contains('\u{2014}'), "an em dash, not a hyphen");
    }

    #[test]
    fn a_press_schedule_reads_like_one() {
        let p = parse_presses(Some("start@120,a@240")).unwrap();
        assert_eq!(p.len(), 2);
        assert_eq!((p[0].frame, p[0].buttons), (120, viper_apu::nes::BTN_START));
        assert_eq!((p[1].frame, p[1].buttons), (240, viper_apu::nes::BTN_A));
        assert!(p[0].hold > 1, "held long enough that a game polling once a frame sees it");
        // A bare button means "soon", since that is what you want most of
        // the time and remembering a frame number is the fiddly part.
        assert_eq!(parse_presses(Some("start")).unwrap()[0].buttons, viper_apu::nes::BTN_START);
        assert!(parse_presses(None).unwrap().is_empty());
        // And a typo is named rather than silently ignored, because a
        // capture that hears nothing is otherwise very hard to explain.
        let err = parse_presses(Some("turbo@10")).unwrap_err().to_string();
        assert!(err.contains("turbo") && err.contains("not a button"), "{}", err);
        assert!(parse_presses(Some("start@soon")).unwrap_err().to_string().contains("frame number"));
    }
...
// ---------- the output other programs read ----------
//
// viper prints for people, but a handful of those lines are also parsed by
// scripts — the downstream album build reads a song's compiled size out of
// the compile summary, and picks generated songs by the header comment
// `viper gen` leaves in each file. Neither is a documented interface and
// there is no `--json`, so the wording *is* the contract.
//
// They live here, named, rather than inline in the middle of a function, so
// that the contract has somewhere to be tested and so that changing it is a
// visible act rather than a passing edit. The tests below say which
// consumers depend on what. That list is incomplete by construction: it
// covers what someone wrote down, not every script in the world.

/// The marker every `viper gen` header comment starts with. Scripts find
/// generated songs by matching this exact prefix, em dash included.
pub const GEN_HEADER_MARKER: &str = "# generated by viper gen — style ";

/// The two comment lines `viper gen` writes at the top of each song.
pub fn gen_header_comment(
    style_name: &str,
    style_version: &str,
    seed: u64,
    key_name: &str,
    info: &crate::style::GenInfo,
    r: &crate::style::Report,
) -> String {
    format!(
        "{}{} v{} seed {} key {} progression {} form {} motif {} shape {} timbre {}\n\
         # {} bars, {} unique phrases, lead density {:.2}, lead range {} semitones, {} drum hits\n",
        GEN_HEADER_MARKER,
        style_name,
        style_version,
        seed,
...
/// The line `viper compile` prints when it has written a file. The two byte
/// counts inside the parentheses are what a packer needs to decide how many
/// songs fit in one 32 KB image.
pub fn compile_summary_line(songs: usize, out: &str, total: usize, data: usize, samples: usize, frames: u32) -> String {
    format!(
