...
- **Stage 12** ✅ — Modulation bindings. A small expression language ties audio
  ...
  Targets (MVP): `x`, `y` (additive pixel offsets on top of the base
  ...
  Deferred: `rotate`, `hue`, `saturation`, `value`, `palette` — those need
  per-pixel color transforms and are a natural Stage 12.1.
